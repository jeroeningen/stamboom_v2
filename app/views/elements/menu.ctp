<?php
	...
	if (empty($authUser) && ($this->name == 'People' && $this->action != 'login')) {
		echo $html->link('Login Libertijnen', array('controller' => 'people', 'action' => 'login', 'admin' => 0), array('onclick' => $modalbox)).' ';
	}
	
	//set menu for logged in user
	if ($this->name == 'People' && !empty($authUser) && $authUser['User']['username'] == 'guest') {
		echo $html->link('Wijzig je gegevens', array('controller' => 'people', 'action' => 'edit', 'admin' => 0, 'id' => null), array('onclick' => $modalbox)).' ';
